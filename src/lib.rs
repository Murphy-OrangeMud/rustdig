use quote::quote;
use syn::{DeriveInput, parse_macro_input, Data, Fields, Field, Type, TypeSlice};
use std::fmt::Debug;
...
pub fn derive_serializer(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    if let Data::Struct(r#struct) = input.data {
        ...
        if matches!(&fields, Fields::Named(_)) {
            let builder_fields = TokenStream::from_iter(
                fields.iter().map(|field: &Field| {
                    let name = field.clone().ident.unwrap();
                    match &field.ty {
                        Type::Path(path) if path.path.get_ident().is_some_and(|path| path == "u16" || path == "u32") => {
                            quote!(
...
pub fn derive_deserializer(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    if let Data::Struct(r#struct) = input.data {
        let fields = r#struct.fields;
        if matches!(&fields, Fields::Named(_)) {
            let set_fields = TokenStream::from_iter(
                fields.iter().map(|field: &Field| {
                    let name = field.clone().ident.unwrap();
                    if name == "name" {
                        return quote!(
            ...
            let build_fields = TokenStream::from_iter(
                fields.iter().map(|field: &Field| {
                    let name = field.clone().ident.unwrap();
                    quote!(
                        #name,
...
#[test]
fn test() {
    let tokens_input = quote!(struct DNSQuestion {
        name: Vec<u8>,
        type_: u16,
        class: u16,
    }).into();
